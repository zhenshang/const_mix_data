...
@register_model("xstnet_tag")
class XSTNet_Tag(FairseqEncoderDecoderModel):
    ...
    @classmethod
    def build_decoder(cls, args, dict):
        decoder = tagdecoder(args, dict)
    ...
    def build_model(cls, args, task):
        ...
        decoder = cls.build_decoder(args, task.target_dictionary)
...
class tagdecoder(FairseqDecoder):
    def __init__(self, args, dictionary):
        super().__init__(dictionary)
        self.tag_projection =  nn.Linear(
                args.decoder_output_dim, 500, bias=False
            ).requires_grad_()
        nn.init.normal_(
                self.tag_projection.weight, mean=0, std=args.decoder_output_dim ** -0.5
            )
    def forward(self, encoder_out):
        
        return self.tag_projection(encoder_out)
...
@register_model_architecture(model_name="xstnet_tag", arch_name="xstnet_tag_base")
