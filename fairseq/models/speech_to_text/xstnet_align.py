...
@register_model("xstnet_align")
class XSTNet_Align(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--load-pretrained-st-encoder-decoder-from",
            type=str,
            metavar="STR",
            help="model to take mt encoder/decoder weights from (for initialization)",
        )
    ...
    def build_model(cls, args, task):
        ...
        st_pretraining_path = getattr(args, "load_pretrained_st_encoder_decoder_from", None)
        # print('***********encoder1***********')
        # parm1 = {}
        # parm2 = {}
        ...
        # print(parm1['transformer_layers.0.self_attn.k_proj.bias'])
        ...
        if st_pretraining_path is not None and Path(st_pretraining_path).exists():
            st_state = checkpoint_utils.load_checkpoint_to_cpu(st_pretraining_path)
            st_encoder_state_dict = OrderedDict()
            st_decoder_state_dict = OrderedDict()
            for key in st_state["model"].keys():
                if key.startswith("encoder"):
                    subkey = key[len("encoder") + 1 :]
                    st_encoder_state_dict[subkey] = st_state["model"][key]
                if key.startswith("decoder"):
                    subkey = key[len("decoder") + 1 :]
                    st_decoder_state_dict[subkey] = st_state["model"][key]
            encoder.load_state_dict(st_encoder_state_dict, strict=False)
            decoder.load_state_dict(st_decoder_state_dict, strict=False)
            logger.info(f"loaded pretrained st encoder and decoder from: {st_pretraining_path}")
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens,
                is_text_input=False, need_all_layer_attn=False, **kwargs):
        if self.training:
            kd_attn = True
        ...
        decoder_out = self.decoder(prev_output_tokens=prev_output_tokens,
                                   encoder_out=encoder_out,need_layer_attn=need_all_layer_attn)
        if self.training:
            

            # print('******************')
            # print(decoder_out[1]['attn'][0].size()) # (bsz, tgt_len, src_len) 
            # print(src_tokens.size())  # (bsz, audio_len) 
            # print(prev_output_tokens.size()) # (bsz, tgt_len) 
            # print(encoder_out.encoder_out.size()) # (src_len, bsz, emb) 
...
class XSTNetEncoder(FairseqEncoder):
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if self.use_ctc:
            ...
            if self.use_audio_tag:
                self.tag_layer = False
                self.audio_tag_layer_projection = nn.Linear(
                    self.encoder_embed_dim, 2000, bias=False
                )
...
@register_model_architecture(model_name="xstnet_align", arch_name="xstnet_align_base")
def base_architecture(args):
    args.audio_tag = getattr(args, "use_audio_tag", False)# default is false, 'store_true'
    args.audio_tag_w2v = getattr(args, "use_audio_tag_w2v", False)# default is false, 'store_true'
