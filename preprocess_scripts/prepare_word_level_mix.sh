...
python3 examples/speech_to_text/seg_mustc_data.py --data-root data/mustc/ --task st --lang ${lang} --output data1/mustc/en-${lang}/segment/tst-COMMON --split tst-COMMON
...
python3 -u merge-data.py --data-root data/mustc/en-de/ --task st --lang ${lang} --split train
...
python3 -u build_dict.py --data-root data/mustc/en-de/ --task st --lang ${lang} --split train --dict-save-path word_audio_dict
python3 -u word-data/build_dict.py --data-root data/mustc/en-de/ --task st --lang de --split train --dict-save-path word_audio_dict

# 最终模型训练时，使用step1生成的train_raw